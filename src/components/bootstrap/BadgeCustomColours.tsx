...
export interface BadgeCustomColoursProps extends BsPrefixProps, React.HTMLAttributes<HTMLElement> {
	bg?: Variant;
	pill?: boolean;
	text?: Color;
}
...
const BadgeCustomColours: BsPrefixRefForwardingComponent<"span", BadgeCustomColoursProps> =
	React.forwardRef<HTMLElement, BadgeCustomColoursProps>(
		(
			{ bsPrefix, bg = "blue", pill = false, text, className, as: Component = "span", ...props },
			ref
		) => {
			...
			return (
				<Component
					ref={ref}
					{...props}
					className={classNames(className, prefix, pill && `rounded-pill`, text && `text-${text}`)}
					style={{
						backgroundColor: bg,
					}}
				/>
